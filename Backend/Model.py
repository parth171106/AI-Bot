...
def FirstLayerDMM(prompt: str = "test"):
    try:
        ...
        # If response is empty, default to general query
        if not response or len(response) == 0:
            print("Warning: No valid decision found, defaulting to general query")
            return [f"general {prompt}"]

        # If 'query' is in the response, recursively call the function for further clarification.
        # Check if any item in the response list contains "(query)"
        if any("(query)" in str(item) for item in response):
        ...
        import traceback
        traceback.print_exc()
        # Return a default general query on error
        return [f"general {prompt}"]
